...
int move_forward(uint8_t sn1, uint8_t sn2, uint8_t snUS, uint8_t snGyro, float gyroval, int max_speed, int distance, int* mode){
    ...
    float value;
    while (( value = get_US_value(snUS)) > distance){
...
int main(void) {
  ...
  // Obtenir la vitesse maximale des moteurs
  ...
  //if possible, modify move_backward to keep going back if ennemy robot moves
}
```
